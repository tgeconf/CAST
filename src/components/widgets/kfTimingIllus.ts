...
export default class KfTimingIllus {
    ...
    /**
     * translate the components in kf or kfgroup
     */
    public transKfDragOffset() {
        const allComponents: any[] = Array.from(this.container.childNodes).slice(1);
        allComponents.forEach((c: any) => {
            if (c.getAttributeNS(null, 'transform')) {
                ...
                c.setAttributeNS(null, 'transform', `translate(${oriTrans.x + this.offsetDiff}, ${oriTrans.y})`);
            } else {
                c.setAttributeNS(null, 'transform', `translate(${this.offsetDiff}, 0)`);
            }
        })
    }
}